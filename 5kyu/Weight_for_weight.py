...
def order_weight(strng):
    # return ' '.join(sorted(sorted(strng.split()), key=sum_digit))
    return ' '.join(sorted(strng.split(), key=sum_digit))
    # return ' '.join(sorted(strng.split(), key=cmp_to_key(cmp_sum_digit)))


# def cmp_sum_digit(s1, s2):
